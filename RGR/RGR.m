...
N = 16; % Количество отсчётов на бит
bits = final_massive; % биты
Tb = 1; % Продолжительность одного бита
...
t_st = linspace(0, (SIGNAL_LENGTH*Tb-1), length(st2));
subplot(4,1,4);
plot(t_st, st2);
ylim([-0.2, 1.2]);
...
subplot(4, 1, 3);
t_st = linspace(0, ((k_corr_end-k_corr_start)*N*Tb-1)/fs, (k_corr_end-k_corr_start)); % Временная шкала для сигнала
plot(t_st, get_signal_massive);
ylim([-0.5, 1.5]);
xlim([1, length(get_signal_massive)]);
xlabel('Время');
ylabel('Амплитуда');
...
legend('Передаваемый сигнал', 'Принимаемый (зашумленный) сигнал');
title('Спектр сигналов через средние символы (8 бит)');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Преобразование числовых кодов в бинарные последовательности
full_name_binary = de2bi(full_name_ascii, 32, 'left-msb'); %32 бит для каждой буквы
full_name_binary = reshape(full_name_binary.', 1, []);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Вычислите CRC длиной M бит для данной последовательности,
%используя входные данные для своего варианта из работы №5 и добавьте к
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Преобразование числовых кодов в бинарные последовательности
full_name_binary = de2bi(full_name_ascii, 7, 'left-msb'); %7 бит для каждой буквы
full_name_binary = reshape(full_name_binary.', 1, []);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Вычислите CRC длиной M бит для данной последовательности,
%используя входные данные для своего варианта из работы №5 и добавьте к
...
ylabel('Амплитуда спектра');
legend('Передаваемый сигнал', 'Принимаемый (зашумленный) сигнал');
title('Спектр сигналов через малые символы (7 бит)');
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%